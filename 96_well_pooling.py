...
from worklist_utils import (worklist_96, target_plate, source_plates, lmgr)
import sys
...
def combine_dispense(a, b):
    c = [None]*2
    c[0] = a[0]+b[0]
    c[1] = a[1]+b[1]
    return c

def list_combine_disp(dl):
    dispense_lists = dl.copy()
    index = 0
    while index < len(dispense_lists)-1:
        if (len(dispense_lists[index][0])+len(dispense_lists[index + 1][0])) <= 8:
            combined_disps = combine_dispense(dispense_lists[index], dispense_lists[index + 1])
            dispense_lists.pop(index)
            dispense_lists[index] = combined_disps
        index += 1
    return dispense_lists

dl = list_combine_disp(dl)
dl = list_combine_disp(dl)
dl = list_combine_disp(dl)


tips = resource_list_with_prefix(lmgr, 'TIP_50ul_L_000', Tip96, 5)
...
if __name__=='__main__':
    with HamiltonInterface(simulate=simulate) as ham_int:
        ...
        for ad_2_tuple in dl:
            ...
            aspirate(ham_int, asp_list, vols, liquidClass = 'Tip_50ul_Water_DispenseJet_Empty')
            dispense(ham_int, disp_list, vols, liquidClass = 'Tip_50ul_Water_DispenseJet_Empty')
            time.sleep(5)
            tip_eject(ham_int)
